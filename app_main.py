...
from itr_modules import NACheckTab, ITRAutofillTab
...
def main() -> None:
    ...
    manager.register_tab("na_check", "NA 自动勾选（Ex ITR NA Check）", NACheckTab)
    ...
    ttk.Button(
        btns,
        text="NA 自动勾选（Ex ITR NA Check）",
        command=lambda: manager.open_tab("na_check"),
        width=30,
    ).pack(side="left")
